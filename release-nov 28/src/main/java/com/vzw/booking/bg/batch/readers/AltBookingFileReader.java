...
public class AltBookingFileReader extends CsvFileGenericReader<AltBookingCsvFileDTO> {
    ...
    public AltBookingFileReader(Environment environment, String filename) {
        super(AltBookingCsvFileDTO.class, environment.getRequiredProperty(PROPERTY_CSV_SOURCE_FILE_PATH).concat(filename), COLUMN_NAMES, ",", 0, ErrorSource.ALTERNATE_BOOKING_INPUT);
    }
    
    public AltBookingFileReader(String filePath) {
        super(AltBookingCsvFileDTO.class, filePath, COLUMN_NAMES, ",", 0, ErrorSource.ALTERNATE_BOOKING_INPUT);
    }
}
